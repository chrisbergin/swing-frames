...
- 2026-07-21: impact frame chase. First diagnosis (tracker lag, impact 2 frames late) was WRONG: it came from comparing pipeline frame indices against frames extracted with cv2 cap.set(CAP_PROP_POS_FRAMES), and seeking is off by 2-3 frames on iPhone HEVC video. Debugging gotcha for this project: always extract reference frames by sequential read, never by seek. Ground truth: hands-lowest picked frame 62, true club-at-ball is 63, one frame early, because a driver strikes the teed ball just after the hands' lowest point and frames 62/63 measure within noise of each other. Fix: refine_impact() re-measures a small window around the tracked impact with IMAGE-mode pose (no temporal prior) and takes the last frame within 5% of the lowest hands. Verified: own video impact tile now shows club head at the ball; step-frame pro clip impact stays inside its impact freeze (window is bounded, so freeze-boundary blend frames are unreachable).
- 2026-07-22: numeric similarity scoring added to --compare mode: 9 joint angles per pose (elbows, shoulders, hips, knees, spine tilt), per-position 0-100 score plus two biggest joint gaps, overall score, saved to similarity_*.json and stamped on the comparison sheet columns. First result vs Horvat driver: overall 71, best address/toe_up (86), worst mid_downswing (46, driven by a 92 deg r_elbow gap that is partly pose noise on the blurriest frame). Caveat: a low score mixes real technique differences with event-timing differences and same-camera-angle is required for scores to mean anything.
