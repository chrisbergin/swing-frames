...
- 2026-07-21: address frame on Driver2 showed the club already in the air. Cause: "last frame near baseline height" lands exactly on the address-to-takeaway cut, and smoothing smears the boundary, so the picked frame is already the takeaway freeze (hands barely move at shaft parallel, only the club does). Fix: step back from that boundary by the smoothing window so address lands inside the still hold. Verified on both videos: club on the ground in both address tiles.
